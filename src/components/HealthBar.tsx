...
interface Props { health: number; maxHealth?: number; }

export const HealthBar: React.FC<Props> = ({ health, maxHealth = 3 }) => (
  <div style={{ display: 'flex', gap: 4, alignItems: 'center' }}>
    {Array.from({ length: maxHealth }).map((_, i) => {
      // 3. srdce (index 2) je "bonusové" — vždy outline, nikdy ztráceno normálně
      // 1. a 2. srdce: plné pokud health > i, šrafované pokud ztraceno
      const isBonusSlot = i === maxHealth - 1;
      const filled = i < health;

      let fillColor: string;
      if (isBonusSlot) {
        // Bonusový slot: plný jen pokud health === maxHealth, jinak vždy outline
        fillColor = health >= maxHealth ? 'var(--red)' : 'none';
      } else {
        fillColor = filled ? 'var(--red)' : `url(#hatch-${i})`;
      }
      ...
      return (
        <svg key={i} width="28" height="28" viewBox="0 0 26 26">
          ...
          <path
            ...
            fill={fillColor}
            stroke={isBonusSlot ? '#9ca3af' : 'var(--ink)'}
            strokeWidth="1.8"
            strokeDasharray={isBonusSlot ? '3 2' : undefined}
          />
        </svg>
      );
    })}
